import { arrayFindIndex } from '../../../YTJS/utils/util';
import { getCell, getColumnByCell, getRowIdentity } from './util';
import { getStyle, hasClass, removeClass, addClass } from '../../../YTJS/utils/dom';
import ytCheckbox from '../../checkbox';
import ytTooltip from '../../tooltip';
...
export default {
    ...
    return (
      <table
        ...
        <tbody>
          ...
          <yt-tooltip effect={ this.table.tooltipEffect } placement="top" ref="tooltip" content={ this.tooltipContent }></yt-tooltip>
        </tbody>
      </table>
    );
  },
  ...
  watch: {
    ...
    'store.states.hoverRow'(newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll('.yt-table__row');
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      const classes = ['yt-table__row'];
      if (this.table.highlightCurrentRow && row === this.store.states.currentRow) {
        classes.push('current-row');
      }

      if (this.stripe && rowIndex % 2 === 1) {
        classes.push('yt-table__row--striped');
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (!(hasClass(cellChild, 'yt-tooltip') && cellChild.childNodes.length)) {
        return;
      }
    ...
    rowRender(row, $index, treeRowData) {
      ...
      let display = true;
      if (treeRowData) {
        rowClasses.push('yt-table__row--levyt-' + treeRowData.level);
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        return [[
          ...
          <tr key={'expanded-row__' + tr.key}>
            <td colspan={ this.columnsCount } class="yt-table__expanded-cell">
