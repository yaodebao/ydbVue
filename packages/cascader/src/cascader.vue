<template>
  <div
    ...
    <yt-input
      ...
      <template slot="suffix">
        <i
          ...
          key="clear"
          class="yt-input__icon yt-icon-circle-close"
    ...
    <div v-if="multiple" class="yt-cascader__tags">
      <yt-tag
      ...
      <input
        ...
        type="text"
        class="yt-cascader__search-input"
    ...
    <transition name="yt-zoom-in-top" @after-leave="handleDropdownLeave">
      <div
        v-show="dropDownVisible"
        ref="popper"
        :class="['yt-popper', 'yt-cascader__dropdown', popperClass]">
        <yt-cascader-panel
          ref="panel"
          v-show="!filtering"
          ...
          @close="toggleDropDownVisible(false)"></yt-cascader-panel>
        <yt-scrollbar
          ref="suggestionPanel"
          v-if="filterable"
          v-show="filtering"
          tag="ul"
          class="yt-cascader__suggestion-panel"
          view-class="yt-cascader__suggestion-list"
          ...
          <template v-if="suggestions.length">
            <li
              ...
              :key="item.uid"
              :class="[
                'yt-cascader__suggestion-item',
                item.checked && 'is-checked'
              ]"
              :tabindex="-1"
              @click="handleSuggestionClick(index)">
              <span>{{ item.text }}</span>
              <i v-if="item.checked" class="yt-icon-check"></i>
            </li>
          </template>
          <slot v-else name="empty">
            <li class="yt-cascader__empty-text">{{ t('el.cascader.noMatch') }}</li>
          </slot>
        </yt-scrollbar>
      </div>
    </transition>
...
import ytCascaderPanel from '../../cascader-panel';
import AriaUtils from '../../../YTJS/utils/aria-utils';
import { t } from '../../../YTJS/locale';
import { isEqual, isEmpty, kebabCase } from '../../../YTJS/utils/util';
import { isUndefined, isFunction } from '../../../YTJS/utils/types';
import { isDef } from '../../../YTJS/utils/shared';
...
export default {
  ...
  computed: {
    realSize() {
      const _elFormItemSize = (this.ytFormItem || {}).ytFormItemSize;
  ...
  methods: {
    ...
    focusFirstNode() {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          firstNode = suggestionPanel.$el.querySelector('.yt-cascader__suggestion-item');
        } else {
          const firstMenu = popper.querySelector('.yt-cascader-menu');
          firstNode = firstMenu.querySelector('.yt-cascader-node[tabindex="-1"]');
        }
    ...
    updateStyle() {
      ...
      const inputInner = $el.querySelector('.yt-input__inner');

      if (!inputInner) return;

      const tags = $el.querySelector('.yt-cascader__tags');
      let suggestionPanelEl = null;

      if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelEl.querySelector('.yt-cascader__suggestion-list');
