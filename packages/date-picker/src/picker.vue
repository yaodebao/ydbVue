<template>
  <yt-input
    class="yt-date-editor"
    :class="'yt-date-editor--' + type"
  ...
  <div
    class="yt-date-editor yt-range-editor yt-input__inner"
    :class="[
      'yt-date-editor--' + type,
      pickerSize ? `yt-range-editor--${ pickerSize }` : '',
    ...
    v-clickoutside="handleClose"
    v-else>
    <i :class="['yt-input__icon', 'yt-range__icon', triggerClass]"></i>
    <input
      autocomplete="off"
      ...
      @focus="handleFocus"
      class="yt-range-input">
    <slot name="range-separator">
      <span class="yt-range-separator">{{ rangeSeparator }}</span>
    </slot>
    <input
      autocomplete="off"
    ...
    <i
      ...
      v-if="haveTrigger"
      :class="[showClose ? '' + clearIcon : '']"
      class="yt-input__icon yt-range__close-icon">
    </i>
  </div>
</template>
...
import { formatDate, parseDate, isDateObject, getWeekNumber } from '../../../YTJS/utils/date-util';
...
export default {
    ...
    pickerVisible(val) {
      ...
      if (val) {
        ...
        if (this.validateEvent) {
          this.dispatch('ytFormItem', 'yt.form.blur');
        }
        this.$emit('blur', this);
        this.blur();
      }
  ...
  computed: {
    ...
    triggerClass() {
      return this.prefixIcon || (this.type.indexOf('time') !== -1 ? 'yt-icon-time' : 'yt-icon-date');
    },
